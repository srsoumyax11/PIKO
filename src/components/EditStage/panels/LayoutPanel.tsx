    ...
    <div>
      ...
      {photos.map(p => (
        <div key={p.id} style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
          <div style={{ width: "36px", height: "48px", borderRadius: "4px", overflow: "hidden", flexShrink: 0, backgroundColor: "#f0f0f0", opacity: p.printCopies > 0 ? 1 : 0.35, transition: "opacity 0.2s" }}>
            <img src={getDisplayUrl(p)} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
          </div>
          <span style={{ fontSize: "11px", color: p.printCopies > 0 ? "var(--ink)" : "var(--steel)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", transition: "color 0.2s" }}>
            {p.originalFile.name}
          </span>
          <input
            type="number" min={0} max={99} value={p.printCopies}
            onChange={e => updatePhoto(p.id, { printCopies: Math.max(0, Number(e.target.value)) })}
            style={{ width: "52px", padding: "4px 8px", textAlign: "center", border: `1px solid ${p.printCopies > 0 ? "var(--focus)" : "var(--line)"}`, borderRadius: "var(--radius-s)", fontFamily: "'Inter',sans-serif", backgroundColor: "var(--paper)", fontWeight: p.printCopies > 0 ? "600" : "400" }}
          />
        </div>
      ))}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "12px" }}>
        <div>
          ...
          <input type="number" min={0} max={20} step={0.5} value={printSettings.borderMm || 0} onChange={e => updatePrintSettings({ borderMm: Number(e.target.value) })} style={{ ...INP, padding: "8px", boxSizing: "border-box" }} />
        </div>
        <div>
          <ColorPicker
            label="Color"
            value={printSettings.borderColor || "#ffffff"}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "12px" }}>
        <div>
          ...
          <input type="number" min={0} max={30} value={printSettings.marginMm} onChange={e => updatePrintSettings({ marginMm: Number(e.target.value) })} style={{ ...INP, padding: "8px", boxSizing: "border-box" }} />
        </div>
        <div>
          <label className="panel-section-label" style={{ fontSize: "11px", marginBottom: "6px", display: "block" }}>Gap (mm)</label>
          <input type="number" min={0} max={20} value={printSettings.gapMm} onChange={e => updatePrintSettings({ gapMm: Number(e.target.value) })} style={{ ...INP, padding: "8px", boxSizing: "border-box" }} />
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px", marginTop: "16px" }}>
        ...
        <Button
          variant={printSettings.showCutLines ? "primary" : "ghost"}
          onClick={() => updatePrintSettings({ showCutLines: !printSettings.showCutLines })}
          style={{ padding: "4px 12px", fontSize: "12px" }}
        >
          {printSettings.showCutLines ? "On" : "Off"}
        </Button>
      </div>
